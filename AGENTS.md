...
- **带宽敏感模块独占**（`EXCLUSIVE_NET_MODULE_KEYS`，v1.12.2）：并行模式
  下 speedtest/bufferbloat/iperf3/tcpcc 不参与并发——普通模块并发结束后
  逐个串行跑（否则测速被网页下载/LAN 扫描等并行流量压低，全量报告与
  单独测速口径不一致）。新增对网络吞吐/并发有影响的模块时评估是否入集。
