...
| `--parallel` | 多模块并行执行 (输出经线程锁同步, 详细结果仍按模块顺序排列); 交互菜单多模块默认并发; 测速/Bufferbloat/iperf3/TCP 并发独占串行跑 (v1.12.2) |
...
_migrate_pcap.py    v1.9.9 旧抓包迁移工具: 修正截断 pcap 的 IP 长度 (Wireshark 复核用)
