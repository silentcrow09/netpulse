...
并行模式带宽敏感模块独占跑：修复全量报告测速被其它并行模块流量压低、
与单独测速报告不一致。

### 修改

- 并行 runner（`_run_diagnostics_parallel`）拆分两阶段：普通模块先并发
  跑完，`EXCLUSIVE_NET_MODULE_KEYS`（speedtest / bufferbloat / iperf3 /
  tcpcc）再逐个串行独占跑——测速的下载/上传窗口不再被网页体检下载、
  LAN 扫描等并行流量抢带宽，全量口径与单独运行口径一致；顺序模式
  （交互菜单单选 / CLI 单模块）不受影响

### 测试

- 新增 `tests/test_v1220_exclusive_net_modules.py`（5 用例：独占集合写死 /
  全量口径普通模块先跑完 / 独占模块彼此不重叠 / 结果归集不受分阶段影响 /
  无独占模块时并发行为不变——Barrier 断言）；全量 21 测试文件 +
  `_smoke_report.py` 250 项断言通过

## [1.12.1] - 2026-09-14

web 模块「慢但通」补盲：网页全部可达但很慢时不再 0 扣分。

### 修改

- **网页体检慢判定升格进 issue 体系**：此前全部目标可达时再慢也不产生
  issue（模块状态「完成」，健康分 0 扣分；平均首字节 ≥500ms 仅给
  assessment 文案「网页访问偏慢」）。现在——平均首字节 ≥2s → 异常
  （-20）、≥500ms → 警告（-2）；新增分段慢判定：DNS ≥200ms /
  TCP ≥500ms / TLS ≥500ms → 警告。阈值与报告配色
  （`THRESHOLDS["web"]` / `_metrics_web`）同值，改动须三处同步
- assessment 新增「网页访问严重缓慢」档（平均首字节 ≥2s）
- `_issues_web` 为新 issue type（`web_slow` / `dns_slow` / `tcp_slow` /
  `tls_slow`）补可执行建议

### 测试

- 新增 `tests/test_web_slow_issues.py`（8 用例：阈值边界 / 分段慢 /
  失败断层路径不受影响 / 严重度→模块状态映射）；全量 21 测试文件 +
  `_smoke_report.py` 250 项断言通过
...
- **菜单 0 / CLI `--modules all` 包含压力级 tcpcc**：压力级语义收窄为
  "仅 debug-bundle 自动全诊断排除"（静默场景不制造 NAT 压测负载，审计
  §12 红线保持）；用户显式点名的"全部"走新增 `full_module_keys()`
  （23 模块全量，无需再确认或输入）。`_stress_excluded_hint` 随之删除
- **tcpcc 默认阶梯上限 1600 → 4000**：旧值对主流家用 NAT（典型 1024~8192）
  明显偏小，常出现"capped 全级别通过"测不出真实容量；硬上限 8000 不变，
  模块超时随级数自动伸缩（8 级 → 136s）
- **tcpcc 多目标轮转压测**：预检通过的公网候选（AliDNS/DNSPod/114，TCP 53）
  **全部入池**，压测连接按 round-robin 分配到池内各端点——单 IP 承受的
  建连压力 ≈ need/N，避免几千条连接集中打同一个 IP 被对方限流/拉黑；
  `--tcpcc-target` 显式指定时保持单目标行为不变；结果新增 `target_pool`
  字段，`target` 汇总标签形如 "3 目标轮转: 223.5.5.5:53+..."。成功率与
  失败分类仍按全池聚合，诊断口径（NAT 上限/瓶颈定位）不受目标个数影响

### 修复

- **菜单 `is_all` 判定失效**（v1.8.1 引入）：`len(keys) == len(MODULE_REGISTRY)`
  在 0 返回 22 项后恒为 False，导致选 0 全量诊断被 port 目标 / web 追加
  站点 / 测速换节点 / iperf3 服务器四个"单选才问"的交互连续打断。现与
  `full_module_keys()` 对齐，选 0 恢复"全程不打断、缺参模块自动跳过"

### 测试

- 402 → 417：新增 `tests/test_v1100_full_all.py` 15 个用例（全量/静默
  双口径、菜单 0 含 tcpcc、阶梯/超时随 4000 伸缩、目标池构建含部分通过/
  全不可用边界、round-robin 分配均匀性与单目标回退）
- `_smoke_report.py` 238 → 244（+6: 全量口径/debug-bundle 排除/菜单 0/
  默认 4000/阶梯伸缩/is_all 对齐源码契约）
...
盯障抓包两个功能点，根因建议里的修复命令仍需用户手动开管理员窗口粘贴）。

### 新增

- **根因建议的「管理员修复命令」一键执行**：规则建议中带 `(管理员)` 尾标的
  netsh 命令（目前唯一生产者：MTU 黑洞规则的接口 MTU 修复）在诊断打印完后
  交互式提议执行——输入序号（可逗号多选，去重保序）→ UAC 确认 → 在独立
  `cmd /k` 管理员窗口运行（窗口保持打开可核对结果/错误码，执行完回 NetPulse
  复测）。NetPulse 自身不修改系统配置，命令文本与报告显示逐字一致
- `_extract_admin_fix_commands()`：只提取带 `(管理员)` 标记的命令段，无标记的
  netsh 文本（如 `netsh winhttp show proxy` 查看类）一律不提取，防误执行；
  去重保序、剥离标记与尾部标点
- 挂钩点 `_print_diagnosis()` 尾部：场景完成屏 / CLI `--diagnose` 两条路径
  同享；非 TTY（脚本/管道）与用户跳过时零打扰

### 测试

- `tests/test_elevation.py` 13 → 27 例：提取语义（有/无标记、去重、标点剥离、
  None 安全）/ 一键执行交互（非 TTY 跳过、Enter 跳过、序号执行、多选去重保序、
  非法输入、ShellExecuteW 拒绝降级）/ `_print_diagnosis` 挂钩
- 全套 17 文件 378 用例全绿；`_smoke_report.py` 238 项断言全绿
...
6 项装维级功能无任何菜单入口）。装维不加参数原则收口——关键功能必须在命令行窗口
内可达。CLI 参数与既有行为零变更。

### 新增

- **场景 `[7]` 盯障增加抓包取证询问（`--capture` 的菜单入口）**：时长问完后先跑
  precheck 亮状态行——可用时询问 `[Enter=不开启 / y=事件切片 / f=全程 /
  s=切片+改上限]`（Enter 与 CLI 默认关闭一致；`s` 追问 MB 上限，非法输入回退
  默认 64）；不可用时（非管理员/无 Npcap/scapy 禁用）只显客户语言原因 + 回车
  继续，不出"选了也白选"的选择题，统计层照跑。首次隐私确认沿用既有
  `.capture_ack` 落标记逻辑
- **工程师菜单新增 `d` 键：生成脱敏调试包（`--debug-bundle` 的菜单入口）**：
  上报排障主通道；无诊断数据时提示先跑一次诊断（不像 CLI 那样自动跑全诊断，
  避免 30-120 秒意外等待），有数据时输出到报告目录
- **单选模块 JIT 询问三处**（仿 `_prompt_for_port_targets` 已验证范式，仅
  单选且 TTY 时触发，Enter=安全默认）：网页体检追加站点
  （`--web-target`，上限 5 个）；测速换节点（`--speedtest-node`，数字 ID 同步
  切 `ookla_server_id`，一次会话只问一次）；iperf3 测速口径 TCP/UDP
  （`--iperf3-udp`，服务器已具备时追问）

### 明确不做（防菜单膨胀，理由见设计稿 §6）

- `--monitor-target / --monitor-load / --load-url` 不进菜单——保住工程师 `m`
  键"零输入直接跑"的刻意设计
- 调优旋钮（port-force/timeout/concurrency、tcpcc-max、iperf3-duration 等
  17 项）维持 CLI-only

### 测试

- `tests/test_v196_menu_entry.py`（19 项）：抓包询问四分支 + 非 TTY/不可用
  降级、场景 `[7]` 透传、`d` 键两态、web/测速节点/iperf3 口径 JIT 接线
- 全量回归 343 项通过
